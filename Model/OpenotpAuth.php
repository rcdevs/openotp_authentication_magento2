...
EOT;
        ...
        if ($u2fChallenge) {
            $overlay .= "+ '<tr style=\"border:none; background:none\"><td id=\"inputs_cell\" style=\"text-align:center; padding-top:5px; border:none;\"><input type=\"hidden\" id=\"openotp_u2f\" name=\"openotp_u2f\" value=\"\">'";
            if ($otpChallenge) {
		...
		function count()
		{
			...
			if( document.body.contains(document.getElementById('openotp_password')) ){
			...
			if(c == 0 || c < 0) {
				...
				document.getElementById("inputs_cell").innerHTML = "<input style='padding:3px 20px;' type='button' value='Retry' class='button mainaction' onclick='window.location.href=\"$backendurl\"'>";
			}
			c--;
		}
		count();
        ...
        if ($u2fChallenge) {
            $overlay .= "if (/chrome|chromium|firefox|opera/.test(navigator.userAgent.toLowerCase())) {
			    ...
			    u2f.sign(u2f_request.appId, u2f_request.challenge, u2f_regkeys, function(response) {
					if(document.getElementById('openotp_u2f')){
						document.getElementById('openotp_u2f').value = JSON.stringify(response); 
